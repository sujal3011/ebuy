...
const Navbar = ({ usertoken, logout, cart, addToCart, removeFromCart, clearCart, cartPrice, reduceItemQuantityFromCart }) => {
    ...
    return (
        ...
        <header className="bg-cyan-900 text-white body-font sticky top-0 z-20">
            <div className="container mx-auto flex flex-wrap p-5 flex-col md:flex-row items-center">
                ...
                <div className="inline-flex items-center cursor: pointer py-1 px-3 focus:outline-none text-base mt-2 md:mt-0 z-0" onMouseOver={()=>{setDropdown(true)}} onMouseLeave={()=>{setDropdown(false)}}>
                    ...
                    {usertoken && dropdown && <div id="dropdown" className="z-10 absolute right-11 top-12 bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700">
                        <ul className="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
                            ...
                            <li>
                                <hr/>
                                <button onClick={logout} className=" w-full block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Sign out</button>
                            </li>
                        </ul>
                    </div>}
